...
Healenium's 0.77 false-heal includes 30 utility-class cases where its proxy is never invoked (baseline selector behaviour — see below), not active mis-heals.
...
To test whether the §6.2 result is specific to one application or one model, we repeated the benchmark on a second complex public application — **Grafana** (an observability dashboard UI, run locally with anonymous admin) — and with a second, **open-weights** model family — Llama-derived **Hermes-3** served locally via Ollama — as a drop-in replacement for the LLM DOM-healer.

**Table 4.** LLM DOM-healing generalizes across two applications and two model families (correct-element recovery; each resolver's own per-application scored set — Supabase N=56 here vs the four-resolver *common* N=53 of Table 2, both rounding to 62% for the Claude healer).

| Resolver | Supabase Studio | Grafana |
|---|---:|---:|
| brittle-only (broken selector) | 0% (0/56) | 0% (0/80) |
| text-role heuristic | 34% (19/56) | 20% (16/80) |
| LLM DOM-healer — Claude Sonnet 4.6 (hosted) | 62% (35/56) | 68% (54/80) |
| LLM DOM-healer — Hermes-3 (open-weights, local) | 64% (36/56) | 55% (44/80) |
