...
**Evaluation scope.** Only the web modality is evaluated in this article, and only as a calibration study (§6.1). Mobile and hardware-in-the-loop are instantiations of the target-agnostic pattern (§2, §5.1): the per-tier driver layer changes (Appium-compatible for mobile, fixture protocols for hardware) while the substrate and event schema are unchanged. Evaluating those modalities live — and replacing the calibration study with production-grade web targets and an adversarial healing benchmark — is the empirical program set out in §7.2.
...
**Scope on a single small public target.** The §6.1 web walkthrough is one small TodoMVC application used as a calibration environment; it is below production scale and establishes nothing about generality beyond its corpus. Healing rates, visual-assertion accuracy, and routing behavior are sensitive to application complexity, target modality, team size, and engineering culture. A three-to-five-application study on production-grade web targets — with an adversarial locator-perturbation benchmark and a baseline comparison (e.g. Healenium) — is the most important next experiment (§7.2). The evaluation is also not benchmark-scale evidence in the SWE-bench sense [33] (Jimenez et al.'s 2,294 multi-repo issue-resolution corpus): the contribution is the cross-layer *coupling pattern*, not an agent's solve-rate on a fixed benchmark, and SWE-bench does not exercise the execution-tier or visual-assertion layers the harness is about.
