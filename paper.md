...
The §6.1 calibration run reports healing only as conditional success on a handful of events, which carries no inferential weight. To measure the healing cascade with a real denominator and ground truth, we built a perturbation benchmark on a production-grade public target — Supabase Studio, a complex React administration UI (dashboards, data grids, icon-only buttons, dynamically generated class names) — run locally.

**Method.** A generator records, per interactive element, a selector-independent ground-truth descriptor (tag, role, visible text, ARIA label), then breaks the element's brittle selector at five controlled difficulty bands: class rename, id/test-id rename, DOM restructuring, sibling re-text, and attribute stripping/minification. A scorer applies each mutation and asks a resolver to recover the element from a semantic query. Recovery counts as a *success* only if the resolved element matches the ground-truth identity, a *false-heal* if a different element is resolved (a silent wrong-element pass — the dangerous case that a passing test hides), and a *miss* if nothing is resolved. To keep the comparison fair on a live application whose DOM drifts between loads, all resolvers are scored on the identical mutated DOM per case. Sixty-five perturbations were scored. Four resolvers are compared: *brittle-only* (the original, now-broken selector — the floor); *Healenium* [16], the industry self-healing-Selenium tool, run through its proxy with a record-then-heal protocol (it stores the element on a clean run and recovers it by DOM tree-similarity when the selector later fails); *text-role* (a vision-free heuristic matching on visible text and role); and the *LLM DOM-healer* (the cascade's healing step via `claude -p` on `claude-sonnet-4-6`, given the broken page's candidate elements and the semantic intent). The artifacts (`chaos/`, including the Healenium Docker stack) regenerate the benchmark from a fresh clone.

**Table 2.** Locator-healing recovery on 65 adversarial perturbations of Supabase Studio (identical mutated DOM per case; recovery requires the *correct* element by ground truth).

| Resolver | Recovery | False-heal | Miss | Median heal time |
|---|---:|---:|---:|---:|
| brittle-only (broken selector) | 8% (5/65) | 0.38 | 0.54 | 17 ms |
| Healenium (self-healing Selenium [16]) | 25% (16/64) | 0.59 | 0.16 | 336 ms |
| text-role heuristic (vision-free) | 39% (25/65) | 0.54 | 0.08 | 2 ms |
| LLM DOM-healer (`claude-sonnet-4-6`) | **63% (41/65)** | 0.23 | 0.14 | ~5.5 s |

The LLM DOM-healer recovers the correct element in 63% of cases against 39% for the text heuristic and 8% for the broken selector, and it is uniquely robust to DOM restructuring: on the restructuring band the text heuristic collapses to 0% while the healer holds at 62% (the healer ranges 62–69% across all five bands). It also carries a lower false-heal rate than the heuristic (0.23 vs 0.54): when uncertain it more often reports a miss — a loud failure — than silently resolving the wrong element. This robustness costs latency: roughly 5.5 s per heal (a live model call) versus ~2 ms for the heuristic. The two approaches are asymmetrically complementary — the healer recovers 19 cases the heuristic misses, while the heuristic recovers only 3 the healer misses.

Healenium, by contrast, recovers only 25% overall, and the breakdown is instructive: it heals on selectors that *truly* fail to match — 47% on the `#id` targets, where a zero-match fires its tree-similarity recovery — but is defeated by utility-class reuse. On the class-selector targets the broken selector silently matches a sibling element, so no zero-match ever fires and Healenium returns the wrong element (0/30, all false-heal). This is precisely the failure mode the intent-based healer avoids, because it resolves from the element's described purpose rather than from the broken selector; it is also why algorithmic selector-healing degrades on modern utility-class UIs. Healenium's heal is fast (336 ms median) relative to the live-model call.

**What this establishes, and what it does not.** With ground truth and a real denominator, the benchmark shows that LLM-based healing materially outperforms a brittle selector and a text heuristic on a complex live UI and is robust to structural change — replacing the non-inferential conditional rates of earlier drafts. It is one application, one model family, 65 perturbations, and a DOM-healer-only proxy for the full cache->DOM->vision cascade; Supabase Studio's UI may also be partly represented in pre-training data (§6.3). Additional production-grade applications and a second (open-weights) model family are the priority extensions (§7.2).
